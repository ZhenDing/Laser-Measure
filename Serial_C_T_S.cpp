...
bool CSerialPort::readFromComm(unsigned char(&readCom)[11] /*Return of Laser*/, int length)
{
	...
	if (BytesInQue != 11)
	{
		...
		std::cout << "收到的数不是11," << BytesInQue <<endl;
		PurgeComm(m_hComm, PURGE_RXCLEAR | PURGE_RXABORT);
		return 0;
	}
	if (BytesInQue == 11)
...
UINT WINAPI CSerialPort::Listen_Laser_Thread(void* pParam)
{
	...
	while (!pSerialPort->s_bExit)
	{
		int ID_Temp = pSerialPort->ID;
		if (ID_Temp > 0)
		{
			if (last_ID != ID_Temp || flag==1)
			{  
				count = 0;
				flag = 0;
				std::cout<<"关闭上一次打开下一次"<<endl;
				if (last_ID != 0)
				{
					pSerialPort->CloseLaser(last_ID - 1);
				}
				Sleep(200);
				pSerialPort->_ConMeaLaser(ID_Temp - 1);
				last_ID = ID_Temp;
			}
			resu = pSerialPort->readFromComm(cRecved, 4);
			if (resu == 1)
			{
				count = 0;
				...
				if (res_de)
				{
					pSerialPort->Distance = Result;
					std::cout <<"ID:" << ID_Temp <<"," << Result << endl;
				}
				last_ID = ID_Temp;
...
bool CSerialPort::_ConMeaLaser(int Num)
{
	unsigned char* ConMea = new unsigned char[4];
	ConMea[0] = 0x80;
	ConMea[1] = 0x06;
	ConMea[2] = 0x03;
	ConMea[3] = 0x77;
...
{
	if (int(cRecved[1]) == 6 && int(cRecved[2]) == 131)
	{
		if (int(cRecved[3]) > 57)
		{
